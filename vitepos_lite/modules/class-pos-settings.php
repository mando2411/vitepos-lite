...
class POS_Settings extends Vitepos_Module {
			...
			<tr class="form-field">
				...
				<td>
					...
					<select name="outlets[]" id="user_select" multiple>
						...
						<?php
						vitepos_get_select_options( $outlets, $selected_outlets );
						?>
					</select>
	...
	public function on_init() {
		...
		add_action( 'vitepos-client-header', array( $this, 'client_header_inline' ), 2 );
	...
	/**
	 * The get product barcode is generated by appsbd
	 *
	 ...
	 *
	 * @param \Appsbd_Lite\V5\Core\any $current_version Its current version param.
	 * @param \Appsbd_Lite\V5\Core\any $previous_version Its previous version param.
	 */
	...
	public static function get_settings( $type = '' ) {
		 ...
		 * @since 3.0.4
		 */
		$basic_settings['is_incl_tax'] = vitepos_apply_filters( 'woocommerce_prices_include_tax', get_option( 'woocommerce_prices_include_tax' ) === 'yes' );
	...
	public function load_pos_client() {
		...
		wp_register_script( 'vitepos-inline-handler', false );
	...
	/**
	 * The client header inline is generated by appsbd
	 */
	public function client_header_inline() {
		$this->get_inline_script( 'vitepos-inline-handler' );
	}

	/**
	 * The pos inline js is generated by appsbd
	 */
	public function pos_inline_js() {
		ob_start();
		?>
		...
		login_type:"<?php echo esc_html( strtoupper( self::get_module_option( 'login_type' ) ) ); ?>",
		ca_prefix:"<?php echo esc_attr( hash( 'crc32b', site_url() ) . '_' ); ?>",
		pos_link:"<?php echo esc_html( self::get_module_instance()->get_pos_link( true ) ); ?>",
		...
		//
		eod_data: vitePosBase + 'outlet/end-of-day-data',
		...
		// Product Attribute
		get_category: vitePosBase + 'product/get-category',
		get_attributes: vitePosBase + 'product/get-attributes',
		add_attribute: vitePosBase + 'product/add-attribute',
		get_attribute: vitePosBase + 'product/get-attribute',
		update_attribute: vitePosBase + 'product/update-attribute',
		delete_attribute: vitePosBase + 'product/delete-attribute',
	...
	/**
	 * The get inline script is generated by appsbd
	 *
	 * @param mixed $handle It is handle param.
	 * @param mixed $position It is position param.
	 */
	public function get_inline_script( $handle, $position = 'after' ) {
		global $wp_scripts;
		if ( isset( $wp_scripts->registered[ $handle ] ) ) {
			$data = $wp_scripts->get_inline_script_data( $handle, $position );
			wp_print_inline_script_tag( $data, array( 'id' => "{$handle}-vitepos" ) );
		}
	}
	...
	public function client_header() {
		...
		$color_ver = $this->get_asset_version( 'templates/pos-assets/css/color-default.css' );
		...
		$links = array(
			...
			array(
				'href' => $this->get_plugin_url( 'templates/pos-assets/css/color-default.css?v=' . $color_ver ),
				'rel'  => 'stylesheet',
			),
			array(
	...
	public function show_order_meta( $order_id ) {
		...
		if ( $is_vite_post ) {
			 ...
			 * @since 1.0
			 */
			$methods         = vitepos_apply_filters( 'apbd-vitepos/filter/payment-methods', $methods );
