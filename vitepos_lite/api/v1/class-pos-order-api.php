...
class Pos_Order_Api extends API_Base {
	...
	private function make_order_payment( $is_offline = false ) {
		...
		if ( ! empty( $this->payload['items'] ) ) {
			...
			$this->set_order_tax( $order, $total_tax );
	 ...
	 */
	protected function set_order_tax( &$order, $total_tax = 0 ) {
		...
		if ( $is_inclusive_enabled ) {
			...
			$order->set_cart_tax( $payload_total_tax );
		} else {
			try {
				$calculated_tax = $order->get_tax_totals();
				if ( $total_tax > 0 ) {
